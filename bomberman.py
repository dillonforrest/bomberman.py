...
class Game:
    ...
    def __init__(self):
        ...
        self.arena = Arena()
        self.players = [Bomberman(self.arena, WHITE,
                        self.arena.start0), AIBot(self.arena, PINK,
                        self.arena.start1)]
    ...
    def updateGameStats(self, Bomb, Explosion):
        ...
        for player in self.players:
            player.bombs = len([bomb for bomb in Bomb if bomb.owner
                               == player])
            if player.bomb_reset > 0:
                player.bomb_reset -= 1
        Explosion._registry = [expl for expl in Explosion if expl.life
                               > 0]
...
class Bomberman:
    ...
    def isInBounds(self, move):
        r = self.radius
        for (m, border) in ((move[0], game_width - r), (move[1],
                            game_height - r)):
            if m < r or m > border:
                return False
        return True
    ...
    def isHittingBlock(self, new, blocks):
        return not any([block[0] <= new <= block[1] for block in
                       blocks])
    ...
    def isMoveOkay(
        ...
        r = self.radius
        if not self.isInBounds(newmove):
            return False
        if newmove[1] == oldmove[1]:
            data = {
                'new': newmove[0],
                'constant': newmove[1],
                'aisles': arena.vt_aisles,
            ...
            data = {
                'new': newmove[1],
                'constant': newmove[0],
                'aisles': arena.hz_aisles,
                'blocks': arena.vt_aisles,
                }
        if not self.isInAisle(data['new'], data['constant'],
                              data['aisles']):
    ...
    def processKeyboardEvents(self, arena):
        ...
        if keys[pygame.K_SPACE]:
            self.dropBomb(arena)
        elif keys[pygame.K_UP] and self.isMoveOkay((x, y - s), (x, y),
                arena):
            self.y -= s
        elif keys[pygame.K_DOWN] and self.isMoveOkay((x, y + s), (x,
                y), arena):
            self.y += s
        elif keys[pygame.K_LEFT] and self.isMoveOkay((x - s, y), (x,
                y), arena):
            self.x -= s
        elif keys[pygame.K_RIGHT] and self.isMoveOkay((x + s, y), (x,
                y), arena):
            self.x += s
...
class AIBot(Bomberman):
    ...
    def LALAmove(self, path):
        pixelpath = [(node[0] * b + b / 2, node[1] * b + b / 2)
                     for node in path]
    ...
    def update(self):
        if self.path is None:
            dest = (randint(1, len(self.arena.grid) - 2), randint(1,
                    len(self.arena.grid) - 2))
            ...
            self.path = [(node[0] * b - b / 2, node[1] * b - b / 2)
...
class Bomb:
    ...
    def findBombPos(self, bbman, arena):
        bbx = bbman.x + bbman.radius
        bby = bbman.y + bbman.radius
        pos = [0, 0]
        hz_tiles = arena.bombx
        vt_tiles = arena.bomby
        for (tiles, bb, n) in ((hz_tiles, bbx, 0), (vt_tiles, bby, 1)):
            for tile in tiles:
                if tile[0] < bb <= tile[1]:
                    pos[n] = sum(tile) / len(tile)
        return (pos[0], pos[1])
...
class Explosion:
    ...
    def findExplArea(self, arena):
        ...
        if typee == '2-way hz':
            (hz_blast, vt_blast) = ((self.x - half, self.y - short,
                                    full, b), (0, 0, 0, 0))
        elif typee == '2-way vt':
            (hz_blast, vt_blast) = ((0, 0, 0, 0), (self.x - short,
                                    self.y - half, b, full))
        else:
    ...
    def find2or4WayExpl(self, arena):
        ...
        for (aisles, center, p) in ((arena.bombx, x, 1), (arena.bomby,
                                    y, 0)):
            for n in aisles:
                if n[0] < center < n[1]:
                    if aisles.index(n) % 2 == 0:
                        path[p] = 'open'
                    else:
                        path[p] = 'closed'
        if path[0] == 'open' and path[1] == 'open':
            return '4-way'
        elif path[0] == 'open' and path[1] == 'closed':
            return '2-way hz'
        elif path[0] == 'closed' and path[1] == 'open':
            return '2-way vt'

    def isInExplArea(self, x, y):
        for blast in self.area:
            if blast[0] < x <= blast[0] + blast[2] and blast[1] < y \
                <= blast[1] + blast[3]:
                return True

    def findNearbyBombs(self):
        return [bomb for bomb in Bomb if self.isInExplArea(bomb.x,
                bomb.y)]

    def drawExplosion(self):
        for i in range(2):
            pygame.draw.rect(window, ORANGE, self.area[i])
...
class Arena:
    ...
    def __init__(self):
        self.array = [[(b * (2 * x + 1), b * (2 * y + 1)) for y in
                      range(NB_DOWN)] for x in range(NB_ACR)]
        self.hz_blocks = [(b * (2 * x + 1), b * (2 * x + 2)) for x in
                          range(NB_ACR)]
        self.vt_blocks = [(b * (2 * y + 1), b * (2 * y + 2)) for y in
                          range(NB_DOWN)]
        self.bbmanradius = 10  # pixel radius of bomberman and enemies
        r = self.bbmanradius
        self.hz_aisles = [(2 * x * b + r, (2 * x + 1) * b - r) for x in
                          range(NB_ACR + 1)]
        self.vt_aisles = [(2 * y * b + r, (2 * y + 1) * b - r) for y in
                          range(NB_ACR + 1)]

        # self.bombi = ( start of tile, end of tile, index of tile )

        self.bombx = [(b * x, b * (x + 1)) for x in range(2 * NB_ACR
                      + 1)]
        self.bomby = [(b * y, b * (y + 1)) for y in range(2 * NB_DOWN
                      + 1)]
        self.start0 = (r, r)
        self.start1 = (r, game_height - r)
        self.start2 = (game_width - r, r)
        self.start3 = (game_width - r, game_height - r)
        self.grid = [[0 for y in range(game_height / b)] for x in
                     range(game_width / b)]
        ...
        for x in range(NB_ACR):
            for y in range(NB_DOWN):
                pygame.draw.rect(window, BLUE,
                                 pygame.Rect(self.array[x][y], (b, b)))
